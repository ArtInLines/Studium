...
void hanoi(int n, int start, int goal, int helper, int** towers) {
   int i; int j; i = j = 0;
   printf("Test: %d\n", towers[0][0]);
   printf("Test");

   if (n > 0) {
      hanoi(n - 1, start, helper, goal, towers);
      // i = j = 0;

      // Move disk
      printf("move disk from %d to %d:\n", start, goal);
      // while (towers[start][i] == 0 && i < globalSize) i++;
      // while (towers[goal][j] == 0 && j <= globalSize) j++;
      // towers[goal][j - 1] = towers[start][i];
      // towers[start][i] = 0;

      hanoi(n - 1, helper, goal, start, towers);
      }
   }
...
int main() {
   ...
   globalSize = n;
   int towers[3][n];
   for (int i = 0; i < n; i++) { towers[0][i] = i + 1; towers[1][i] = towers[2][i] = 0; }
